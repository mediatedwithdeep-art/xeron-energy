...
import { megaMenu, site, utilityLinks } from "@/lib/site";

export default function Footer() {
  const year = new Date().getFullYear();
  // Only render icons for accounts that actually exist — a link to a bare
  // social homepage reads as unfinished and is dead weight in search schema.
  const socials = [
      ...
      <div className="container-x relative z-10 py-16 md:py-20">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
          <div>
            ...
            <p className="mt-5 max-w-sm text-sm leading-relaxed text-mist">
              ...
              of electrical engineering experience.
            </p>
            {socials.length > 0 && (
        ...
        <div className="mt-14 grid gap-4 border-t border-white/8 pt-8 sm:grid-cols-3">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-3 text-sm text-mist transition-colors hover:text-solar"
          >
            <FaWhatsapp className="h-5 w-5 text-solar" />
            WhatsApp us
          </a>
        ...
        <div className="mt-8 flex flex-col items-center justify-between gap-3 border-t border-white/8 pt-6 text-xs text-muted-ink sm:flex-row">
          <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2">
            ...
            {utilityLinks.map((l) => (
              <Link key={l.href} href={l.href} className="transition-colors hover:text-solar">
                {l.label}
              </Link>
            ))}
          </div>
